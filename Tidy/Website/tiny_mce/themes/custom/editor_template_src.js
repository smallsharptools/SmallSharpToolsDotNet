...
var TinyMCE_CustomTheme = {
	...
	_buttonMap : 'anchor,bold,bullist,charmap,cleanup,code,copy,cut,help,indent,italic,hr,link,numlist,outdent,paste,redo,removeformat,undo,unlink,advhr,ltr,rtl,emotions,flash,fullpage,fullscreen,iespell,insertdate,inserttime,pastetext,pasteword,selectall,preview,print,save,replace,search',
	...
	getControlHTML : function(button_name) {
		...
		for (i=0; i<TinyMCE_CustomTheme._buttons.length; i++) {
			but = TinyMCE_CustomTheme._buttons[i];
		...
		switch (button_name) {
			case "formatselect":
				...
				var formats = tinyMCE.getParam("theme_custom_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).split(',');
				var lookup = [
			...
			case "styleselect":
				return '<select id="{$editor_id}_styleSelect" onmousedown="tinyMCE.themes.custom._setupCSSClasses(\'{$editor_id}\');" name="{$editor_id}_styleSelect" onfocus="tinyMCE.addSelectAccessibility(event,this,window);" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,this.options[this.selectedIndex].value);" class="mceSelectList">{$style_select_options}</select>';
			...
			case "fontselect":
				...
				var fonts = tinyMCE.getParam("theme_custom_fonts", nFonts).split(';');
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		switch (command) {
			...
			case "mceLink":
				...
				if (inst.settings['insertlink_callback']) {
					...
					if (returnVal && returnVal['href'])
						TinyMCE_CustomTheme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class']);
				} else {
			...
			case "mceImage":
				...
				if (inst.settings['insertimage_callback']) {
					...
					if (returnVal && returnVal['src'])
						TinyMCE_CustomTheme._insertImage(returnVal['src'], returnVal['alt'], returnVal['border'], returnVal['hspace'], returnVal['vspace'], returnVal['width'], returnVal['height'], returnVal['align'], returnVal['title'], returnVal['onmouseover'], returnVal['onmouseout']);
				} else
			...
			case "forecolor":
				...
				TinyMCE_CustomTheme._hideMenus(editor_id);
				...
				if (!fcp.exists()) {
					...
					tinyMCE.addEvent(elm, 'click', TinyMCE_CustomTheme._handleMenuEvent);
					tinyMCE.addEvent(elm, 'mouseover', TinyMCE_CustomTheme._handleMenuEvent);
					tinyMCE.addEvent(elm, 'mouseout', TinyMCE_CustomTheme._handleMenuEvent);
				}
			...
			case "forecolorMenu":
				...
				if (!ml.exists())
					ml.create('div', 'mceMenu', document.body, TinyMCE_CustomTheme._getColorHTML(editor_id, 'theme_custom_text_colors', 'forecolor'));
			...
			case "HiliteColorMenu":
				...
				if (!ml.exists())
					ml.create('div', 'mceMenu', document.body, TinyMCE_CustomTheme._getColorHTML(editor_id, 'theme_custom_background_colors', 'HiliteColor'));
			...
			case "mceColorPicker":
				if (user_interface) {
					...
					template['width'] += tinyMCE.getLang('lang_theme_custom_colorpicker_delta_width', 0);
					template['height'] += tinyMCE.getLang('lang_theme_custom_colorpicker_delta_height', 0);
			...
			case "mceCharMap":
				...
				template['width'] += tinyMCE.getLang('lang_theme_custom_charmap_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_custom_charmap_delta_height', 0);
			...
			case "mceInsertAnchor":
				...
				template['width'] += tinyMCE.getLang('lang_theme_custom_anchor_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_theme_custom_anchor_delta_height', 0);
	...
	getEditorTemplate : function(settings, editorId) {
		...
		var template = new Array();
		var deltaHeight = 0;
		var resizing = tinyMCE.getParam("theme_custom_resizing", false);
		var path = tinyMCE.getParam("theme_custom_path", true);
		var statusbarHTML = '<div id="{$editor_id}_path" class="mceStatusbarPathText" style="display: ' + (path ? "block" : "none") + '">&#160;</div><div id="{$editor_id}_resize" class="mceStatusbarResize" style="display: ' + (resizing ? "block" : "none") + '" onmousedown="tinyMCE.themes.custom._setResizing(event,\'{$editor_id}\',true);"></div><br style="clear: both" />';
		var layoutManager = tinyMCE.getParam("theme_custom_layout_manager", "SimpleLayout");

		// Setup style select options -- MOVED UP FOR EXTERNAL TOOLBAR COMPATABILITY!
		var styleSelectHTML = '<option value="">{$lang_theme_style_select}</option>';
		if (settings['theme_custom_styles']) {
			var stylesAr = settings['theme_custom_styles'].split(';');
			...
			TinyMCE_CustomTheme._autoImportCSSClasses = false;
		}

		switch(layoutManager) {
			case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
				var toolbarHTML = "";
				var toolbarLocation = tinyMCE.getParam("theme_custom_toolbar_location", "bottom");
				var toolbarAlign = tinyMCE.getParam("theme_custom_toolbar_align", "center");
				var pathLocation = tinyMCE.getParam("theme_custom_path_location", "none"); // Compatiblity
				var statusbarLocation = tinyMCE.getParam("theme_custom_statusbar_location", pathLocation);
				var defVals = {
					theme_custom_buttons1 : "bold,italic,bullist,numlist,outdent,indent,separator,styleselect,formatselect",
					theme_custom_buttons2 : "link,unlink,anchor,hr,removeformat,separator,charmap,undo,redo,cleanup,code,help",
				...
				// Render rows
				for (var i=1; i<100; i++) {
					var def = defVals["theme_custom_buttons" + i];

					var buttons = tinyMCE.getParam("theme_custom_buttons" + i, def == null ? '' : def, true, ',');
					if (buttons.length == 0)
						break;

					buttons = removeFromArray(buttons, tinyMCE.getParam("theme_custom_disable", "", true, ','));
					buttons = addToArray(buttons, tinyMCE.getParam("theme_custom_buttons" + i + "_add", "", true, ','));
					buttons = addToArray(tinyMCE.getParam("theme_custom_buttons" + i + "_add_before", "", true, ','), buttons);
			...
			case "RowLayout" : //Container Layout - containers defined in "theme_custom_containers" are rendered from top to bottom.
				template['html'] = '<table class="mceEditor" border="0" cellpadding="0" cellspacing="0" width="{$width}" height="{$height}" style="width:{$width}px;height:{$height}px"><tbody>';

				var containers = tinyMCE.getParam("theme_custom_containers", "", true, ",");
				var defaultContainerCSS = tinyMCE.getParam("theme_custom_containers_default_class", "container");
				var defaultContainerAlign = tinyMCE.getParam("theme_custom_containers_default_align", "center");
				...
				for (var i = 0; i < containers.length; i++)
				{
					...
					else if (containers[i] == "mceElementpath" || containers[i] == "mceStatusbar") // ... mceElementpath:
					{
						...
						var curContainer = tinyMCE.getParam("theme_custom_container_"+containers[i], "", true, ',');
						var curContainerHTML = "";
						var curAlign = tinyMCE.getParam("theme_custom_container_"+containers[i]+"_align", defaultContainerAlign);
						var curCSS = tinyMCE.getParam("theme_custom_container_"+containers[i]+"_class", defaultContainerCSS);

						curContainer = removeFromArray(curContainer, tinyMCE.getParam("theme_custom_disable", "", true, ','));
			...
			case "CustomLayout" : //User defined layout callback...
				var customLayout = tinyMCE.getParam("theme_custom_custom_layout","");
	...
	initInstance : function(inst) {
		if (tinyMCE.getParam("theme_custom_resizing", false)) {
			if (tinyMCE.getParam("theme_custom_resizing_use_cookie", true)) {
				var w = TinyMCE_CustomTheme._getCookie("TinyMCE_" + inst.editorId + "_width");
				var h = TinyMCE_CustomTheme._getCookie("TinyMCE_" + inst.editorId + "_height");

				TinyMCE_CustomTheme._resizeTo(inst, w, h, tinyMCE.getParam("theme_custom_resize_horizontal", true));
			}
		}
	...
	hideInstance : function(inst) {
		TinyMCE_CustomTheme._hideMenus(inst.editorId);
	},
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		if (selectElm) {
			TinyMCE_CustomTheme._setupCSSClasses(editor_id);
	...
	_setupCSSClasses : function(editor_id) {
		...
		if (!TinyMCE_CustomTheme._autoImportCSSClasses)
			return;
	...
	_resizeEventHandler : function(e) {
		var resizer = TinyMCE_CustomTheme._resizer;
		...
		switch (e.type) {
			...
			case "mouseup":
				TinyMCE_CustomTheme._setResizing(e, editorId, false);
				TinyMCE_CustomTheme._resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
				...
				if (tinyMCE.getParam("theme_custom_resizing_use_cookie", true)) {
					...
					// Set the cookies
					TinyMCE_CustomTheme._setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
					TinyMCE_CustomTheme._setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
				}
				break;
		}
	},
	...
	_setResizing : function(e, editor_id, state) {
		...
		if (state) {
			...
			if (!resizer.eventHandlers) {
				if (tinyMCE.isMSIE)
					tinyMCE.addEvent(document, "mousemove", TinyMCE_CustomTheme._resizeEventHandler);
				else
					tinyMCE.addEvent(window, "mousemove", TinyMCE_CustomTheme._resizeEventHandler);

				tinyMCE.addEvent(document, "mouseup", TinyMCE_CustomTheme._resizeEventHandler);
			...
			resizer.horizontal = tinyMCE.getParam("theme_custom_resize_horizontal", true);
		} else {
	...
	_getColorHTML : function(id, n, cm) {
		...
		cl = tinyMCE.getParam(n, TinyMCE_CustomTheme._defColors).split(',');
		...
		if (tinyMCE.getParam("theme_custom_more_colors", true))
			h += '<a href="javascript:void(0);" onclick="TinyMCE_CustomTheme._pickColor(\'' + id + '\',\'' + cm + '\');" class="mceMoreColors">' + tinyMCE.getLang('lang_more_colors') + '</a>';
...
// Add default buttons maps for custom theme and all internal plugins
